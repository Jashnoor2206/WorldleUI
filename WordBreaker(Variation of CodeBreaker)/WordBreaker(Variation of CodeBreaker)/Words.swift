...
class Words {
    ...
    private init(from url: URL? = nil) { // makes the dictionary on based of { length, set of words } pair
        Task {
    ...
    var count: Int { // gives total number of words in the dictionary
        words.values.reduce(0) { $0 + $1.count }
    }
    
    func contains(_ word: String) -> Bool { // returns if a word is present in dictionary or not
        words[word.count]?.contains(word.uppercased()) == true
    }

    func random(length: Int) -> String? { // gives random word of specified lenght if present
...
extension UITextChecker { // used for checking spellings
